...
mod tests {
    ...
    fn copy_all_resources_to_target_directory() {
        ...
        let mut files:Vec<_> = read_dir(&tempdir).unwrap().map(|o| {
            o.unwrap().path()
        }).collect();
        files.sort();
        assert_eq!(&files[0].strip_prefix(&tempdir).unwrap(),&Path::new("certificate.yaml"));
        assert_eq!(&files[1].strip_prefix(&tempdir).unwrap(),&Path::new("test"));
    }
}
